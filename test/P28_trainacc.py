...
# 添加tensorboard
writer = SummaryWriter("../P28_trainacc")
...
for i in range(epoch):   # 0~9
    ...
    for data in train_dataloader:
        ...
        if total_train_step % 100 == 0:   # 训练次数每100次，print loss
            end_time = time.time()
            print("训练次数：{}， loss：{}， 所花费的时间：{}s".format(total_train_step, loss.item(), end_time - start_time))  # .item 将tensor-->真实值
    ...
    # 保存每一轮训练的模型
    torch.save(tudui, "tudui_{}_cpu_acc.pth".format(i))
